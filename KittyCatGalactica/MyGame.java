...
public class MyGame extends VariableFrameRateGame implements MouseListener, MouseMotionListener{
	...
	private Action quitGameAction, incrementCounterAction, moveForwardAction, moveBackwardAction
					...
					volumeMuteAction, speedBoostAction, LightUpAction, LightDownAction;
    ...
    protected void update(Engine engine){
		...
		musicVolume = bgSound.getVolume();
		if(bgSound.getIsPlaying() == false){
			musicVolume = 0;
		}
		...
		if(!winStatus.isEmpty())
			...
			dispStr = "Score:  " + count1 + "  Music:  " + musicVolume;
		...
		bgSound.setLocation(dolphinN.getWorldPosition()); 
		setEarParameters(sm);
	...
	protected void setupInputs(SceneManager sm){
		...
		volumeDownAction = new VolumeDownAction(bgSound, this);
		volumeUpAction = new VolumeUpAction(bgSound, this);
		volumeMuteAction = new VolumeMuteAction(bgSound, this);
	...
	public void initAudio(SceneManager sm){
		...
		resource3 = audioMgr.createAudioResource("assets/sounds/BPM 80 008 C Mjr Techno.wav",AudioResourceType.AUDIO_SAMPLE);
		...
		bgSound = new Sound(resource3,SoundType.SOUND_EFFECT, 10, true);
